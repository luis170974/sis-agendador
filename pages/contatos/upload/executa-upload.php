...
if(isset($_FILES['arquivo']['name'])):
    ...
    if($tamanho_arquivo > $tamanho_bytes): // verificando tamanho do arquivo
        die("Arquivo deve ter no máximo {$tamanho_bytes} bytes.;aviso");
    endif;

    if(!in_array($extensao, $extensoes_validas)): //verificando extensão do arquivo
    ...
    if(move_uploaded_file($arquivo_temporario, "$caminho_absoluto/$nome_arquivo_novo")):
        $sql = "UPDATE tbcontatos SET nomeFotoContato = '{$nome_arquivo_novo}' 
        WHERE idContato = '{$idContato}'";

        mysqli_query($conexao, $sql);
        
        echo "./pages/contatos/fotos-contatos/{$nome_arquivo_novo};concluido";
